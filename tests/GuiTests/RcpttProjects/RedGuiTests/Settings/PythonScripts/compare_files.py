    ...
    if os.path.exists(sys.argv[1]) and os.path.exists(sys.argv[2]):
        ...
        if result_file and schema_file:
            ...
            if len(result_file) > len(schema_file):
                status.append("Result file ({0}) is bigger than schema file ({1}).".format(str(len(result_file)),
                                                                                           str(len(schema_file))))
                counter = len(schema_file)
            if len(schema_file) > len(result_file):
                status.append("Schema file ({0}) is bigger than result file ({1}).".format(str(len(schema_file)),
                                                                                           str(len(result_file))))
