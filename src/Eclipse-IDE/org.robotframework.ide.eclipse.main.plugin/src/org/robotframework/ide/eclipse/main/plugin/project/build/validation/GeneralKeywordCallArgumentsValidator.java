...
class GeneralKeywordCallArgumentsValidator extends KeywordCallArgumentsValidator {
    ...
    public void validate(final IProgressMonitor monitor) {
        try {
            ...
            final TaggedCallSiteArguments taggedArguments = TaggedCallSiteArguments.tagArguments(arguments,
                    argsByNames, descriptor.supportsKwargs());
    ...
    private void validateNumberOfArgs(final TaggedCallSiteArguments taggedArguments) {
        ...
        final ArgumentProblem cause = descriptor.supportsKwargs()
                ? ArgumentProblem.INVALID_NUMBER_OF_NON_KEYWORD_PARAMETERS
        ...
        if (!possibleRange.contains(actual) && !taggedArguments.containsCollectionArgument()) {
            ...
            final String additional = String.format("Keyword '%s' expects %s but %d %s provided",
                    definingToken.getText(), getRangesInfo(possibleRange, element), actual, toBeInProperForm(actual));
            reporter.handleProblem(RobotProblem.causedBy(cause).formatMessageWith(additional), file, definingToken);
            ...
            if (possibleRange.upperEndpoint() < noOfNonCollectionArgs) {
                ...
                final String element = descriptor.supportsKwargs() ? "non-keyword argument" : "argument";
                final String additional = String.format("Keyword '%s' expects %s but %s %s provided",
                        definingToken.getText(), getRangesInfo(possibleRange, element), getRangesInfo(actualRange, ""),
