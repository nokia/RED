...
import org.robotframework.ide.eclipse.main.plugin.tableeditor.cases.CasesElementsStyleConfiguration;
...
class TasksFormFragment implements ISectionFormFragment {
    ...
    private void setupNatTable(final Composite parent) {
        ...
        dataProvider = new TasksDataProvider(propertyAccessor, getSection());

        final IDataProvider columnHeaderDataProvider = new TasksColumnHeaderDataProvider();
    ...
    private NatTable createTable(final Composite parent, final TableTheme theme, final RedNattableLayersFactory factory,
        ...
        // popup menus
        table.addConfiguration(new TasksTableMenuConfiguration(site, table, selectionProvider));
    ...
    private RobotTasksSection getSection() {
        return fileModel.findSection(RobotTasksSection.class).orElse(null);
    }
    ...
    public void revealElement(final RobotElement element, final boolean focus) {
        if (dataProvider.isFilterSet() && !dataProvider.isProvided(element)) {
            final String topic = RobotSuiteEditorEvents.FORM_FILTER_SWITCH_REQUEST_TOPIC + "/"
                    + RobotTasksSection.SECTION_NAME.replaceAll(" ", "_");
            eventBroker.send(topic, new FilterSwitchRequest(RobotTasksSection.SECTION_NAME.replaceAll(" ", "_"), ""));
        }
        CellEditorCloser.closeForcibly(table);
        if (focus) {
            table.setFocus();
    ...
    private NewElementsCreator<RobotElement> newElementsCreator() {
        return addingTokenRowIndex -> {
            ...
            if (token.isNested()) {
                final RobotTask task = (RobotTask) token.getParent();
                commandsStack.execute(new CreateFreshKeywordCallCommand(task));
                return task.getChildren().get(task.getChildren().size() - 1);
            } else {
                final RobotTasksSection section = dataProvider.getInput();
    ...
    public HeaderFilterMatchesCollection collectMatches(final String filter) {
        final CodeElementsMatchesCollection tasksMatches = new CodeElementsMatchesCollection();
        tasksMatches.collect(dataProvider.getInput(), filter);
        return tasksMatches;
    }
    ...
    private void whenUserRequestedFilteringEnabled(@UIEventTopic(RobotSuiteEditorEvents.SECTION_FILTERING_ENABLED_TOPIC
            + "/Tasks") final HeaderFilterMatchesCollection matches) {
    ...
    private void whenUserRequestedFilteringDisabled(
            @UIEventTopic(RobotSuiteEditorEvents.SECTION_FILTERING_DISABLED_TOPIC
                    + "/Tasks") final Collection<HeaderFilterMatchesCollector> collectors) {
    ...
    @Inject
    @Optional
    private void whenTaskIsAdded(@UIEventTopic(RobotModelEvents.ROBOT_ELEMENT_ADDED) final Event event) {
        final RobotTasksSection section = Events.get(event, IEventBroker.DATA, RobotTasksSection.class);

        if (section != null && section.getSuiteFile() == fileModel) {
            sortModel.clear();

            RobotTask task = Events.get(event, RobotModelEvents.ADDITIONAL_DATA, RobotTask.class);
            if (task == null) {
                final List<?> keywords = Events.get(event, RobotModelEvents.ADDITIONAL_DATA, List.class);
                task = (RobotTask) keywords.get(keywords.size() - 1);
            }

            selectionLayerAccessor.selectElementInFirstCellAfterOperation(task, tableInputIsReplaced());
        }
    }

    @Inject
    @Optional
    private void whenTaskIsRemoved(
            @UIEventTopic(RobotModelEvents.ROBOT_ELEMENT_REMOVED) final RobotTasksSection section) {
    ...
    @Inject
    @Optional
    private void whenTaskIsMoved(@UIEventTopic(RobotModelEvents.ROBOT_ELEMENT_MOVED) final RobotTasksSection section) {
        if (section.getSuiteFile() == fileModel) {
            sortModel.clear();
    ...
    private void whenKeywordCallIsRemoved(
            @UIEventTopic(RobotModelEvents.ROBOT_KEYWORD_CALL_REMOVED) final RobotTask task) {
        if (task.getSuiteFile() == fileModel) {
            selectionLayerAccessor.preserveSelectionWhen(tableInputIsReplaced(), coordinate -> {
                if (task.getChildren().isEmpty()) {
                    return new PositionCoordinate(coordinate.getLayer(), coordinate.getColumnPosition(),
                            dataProvider.indexOfRowObject(task));
                } else if (dataProvider.getRowObject(coordinate.getRowPosition()) instanceof AddingToken) {
                    return new PositionCoordinate(coordinate.getLayer(), coordinate.getColumnPosition(),
    ...
    private void whenKeywordCallIsMoved(
            @UIEventTopic(RobotModelEvents.ROBOT_KEYWORD_CALL_MOVED) final RobotTask task) {
        if (task.getSuiteFile() == fileModel) {
            sortModel.clear();
            selectionLayerAccessor.preserveElementSelectionWhen(tableInputIsReplaced());
        }
    ...
    @Inject
    @Optional
    private void whenTaskDetailIsChanged(
            @UIEventTopic(RobotModelEvents.ROBOT_ELEMENT_NAME_CHANGED) final RobotTask task) {
        if (task.getSuiteFile() == fileModel) {
            table.update();
            table.refresh();
            setDirty();
    ...
    private void whenKeywordCallDetailIsChanged(
        ...
        if (keywordCall.getParent() instanceof RobotTask && keywordCall.getSuiteFile() == fileModel) {
    ...
    private void whenColumnCountIsExceeded(@UIEventTopic(RobotModelEvents.COLUMN_COUNT_EXCEEDED) final Event event) {
        ...
        if (keywordCall != null && keywordCall.getParent() instanceof RobotTask
    ...
    private class TasksColumnHeaderDataProvider extends RedColumnHeaderDataProvider {

        public TasksColumnHeaderDataProvider() {
            super(dataProvider);
        }
    ...
    private static class TasksTableMenuConfiguration extends TableMenuConfiguration {

        public TasksTableMenuConfiguration(final IEditorSite site, final NatTable table,
                final ISelectionProvider selectionProvider) {
            super(site, table, selectionProvider, "org.robotframework.ide.eclipse.editor.page.tasks.contextMenu",
                    "Robot suite editor tasks context menu");
        }
    }
}
