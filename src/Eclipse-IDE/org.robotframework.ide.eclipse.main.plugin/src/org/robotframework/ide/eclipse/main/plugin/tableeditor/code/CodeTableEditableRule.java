 ...
 * @author wypych
 */
public class CodeTableEditableRule extends SuiteModelEditableRule {

    private CodeTableEditableRule(final boolean isEditable) {
        super(isEditable);
    }

    public static IEditableRule createEditableRule(final RobotSuiteFile fileModel) {
        return new CodeTableEditableRule(fileModel.isEditable());
    }

    @Override
    public boolean isEditable(final ILayerCell cell, final IConfigRegistry configRegistry) {
        return super.isEditable(cell, configRegistry)
                && !cell.getConfigLabels().hasLabel(TableConfigurationLabels.CELL_NOT_EDITABLE_LABEL);
    }
}
