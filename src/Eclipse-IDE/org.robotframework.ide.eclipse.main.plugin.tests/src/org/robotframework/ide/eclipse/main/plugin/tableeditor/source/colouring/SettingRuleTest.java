...
public class SettingRuleTest {
    ...
    private final SettingRule testedRule = new SettingRule(new Token("token"));
    ...
    @Test
    public void settingIsRecognized() {
    ...
    @Test
    public void settingIsRecognized_evenWhenPositionIsInsideToken() {
        boolean thereWasName = false;
        for (final RobotToken token : TokensSource.createTokens()) {
    ...
    private boolean isSettingDeclarationToken(final RobotToken token) {
        final RobotTokenType type = RobotTokenType.findTypeOfDeclarationForSettingTable(token.getText());
        return token.getText().equals("UnkownSetting")
                || type != RobotTokenType.UNKNOWN && type != RobotTokenType.SETTING_UNKNOWN;
    }
