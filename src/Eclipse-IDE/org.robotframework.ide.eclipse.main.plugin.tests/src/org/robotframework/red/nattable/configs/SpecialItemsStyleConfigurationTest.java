 ...
 *
 */
public class SpecialItemsStyleConfigurationTest {

    private final RedPreferences preferences = mock(RedPreferences.class);

    @Before
    public void before() {
        when(preferences.getSyntaxColoring(SyntaxHighlightingCategory.SPECIAL))
    ...
    public void sameStyleIsRegisteredForEachDisplayMode() throws Exception {
        ...
        final SpecialItemsStyleConfiguration config = new SpecialItemsStyleConfiguration(mock(TableTheme.class),
                preferences);
        ...
        final IStyle style4 = configRegistry.getConfigAttribute(CellConfigAttributes.CELL_STYLE,
                DisplayMode.SELECT_HOVER, SpecialItemsLabelAccumulator.SPECIAL_ITEM_CONFIG_LABEL);
    ...
    public void fontDefinedInStyleUsesFontTakenFromThemeWithStyleDefinedInPreferences() {
        ...
        final SpecialItemsStyleConfiguration config = new SpecialItemsStyleConfiguration(theme, preferences);
        config.configureRegistry(configRegistry);
