...
public class ResourceImportTest {
    ...
    private ResourceImport testable;

    @Before
    public void setUp() {
        this.decToken = new RobotToken();
        this.testable = new ResourceImport(decToken);
    }
    ...
    public void test_ifCellWillBeAdded_atArgumentPosition() {
        ...
        assertThat(elementTokens.get(0).getTypes()).contains(RobotTokenType.SETTING_RESOURCE_DECLARATION);
        assertThat(elementTokens.get(1).getText()).isEqualTo("value");
        assertThat(elementTokens.get(1).getTypes()).contains(RobotTokenType.SETTING_RESOURCE_UNWANTED_ARGUMENT);
    ...
    public void test_ifCellWillBeAdded_atCommentPosition() {
        ...
        assertThat(elementTokens.get(2).getTypes()).contains(RobotTokenType.SETTING_RESOURCE_UNWANTED_ARGUMENT);
        assertThat(elementTokens.get(3)).isSameAs(cmtToken);
        assertThat(elementTokens.get(3).getTypes()).contains(RobotTokenType.START_HASH_COMMENT);
    }
}
