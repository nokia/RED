...
public class KeywordUnknownSettingsTest {

    private RobotToken decToken;
    private KeywordUnknownSettings testable;

    @Before
    public void setUp() {
        this.decToken = new RobotToken();
        this.testable = new KeywordUnknownSettings(decToken);
    }
    ...
    public void test_ifCellWillBeAdded_atArgumentPosition() {
        ...
        assertThat(elementTokens.get(0).getTypes()).contains(RobotTokenType.KEYWORD_SETTING_UNKNOWN_DECLARATION);
        assertThat(elementTokens.get(1).getText()).isEqualTo("value");
        assertThat(elementTokens.get(1).getTypes()).contains(RobotTokenType.KEYWORD_SETTING_UNKNOWN_ARGUMENTS);
        assertThat(elementTokens.get(2)).isSameAs(argToken);
        assertThat(elementTokens.get(2).getTypes()).contains(RobotTokenType.KEYWORD_SETTING_UNKNOWN_ARGUMENTS);
