...
public class AVariablesTokenRecognizerTest {
    ...
    public void test_createVariablePattern() {
        final Pattern p = AVariablesTokenRecognizer.createVariablePattern(VariableType.SCALAR);

        assertThat(p.toString())
                .isEqualTo("[ ]?[" + VariableType.SCALAR.getIdentificator() + "]" + "(\\s*)[{].*([}]$|$)");
    }
    ...
    public void test_getPattern() {
        ...
        assertThat(rec.getPattern().toString())
                .isEqualTo("[ ]?[" + TYPE.getIdentificator() + "]" + "(\\s*)[{].*([}]$|$)");
    }
