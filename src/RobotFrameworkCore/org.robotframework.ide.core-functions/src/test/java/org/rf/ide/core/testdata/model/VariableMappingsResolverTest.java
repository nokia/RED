...
public class VariableMappingsResolverTest {
    ...
    public void testSkippingNonScalarMappings() {
        ...
        assertThat(resolve(mappings, null)).hasSize(5).containsAllEntriesOf(ImmutableMap.of("${a}", "x", "${d}", "x"));
    }

    @Test
    public void testResolvingMappingsWithoutVariablesInValues() {
        assertThat(resolve(newArrayList(VariableMapping.create("${AbC}", "x/y"),
                VariableMapping.create("${dEf}", "y/z"), VariableMapping.create("${G_H I}", "a/b/c")), null)).hasSize(6)
                        .containsAllEntriesOf(ImmutableMap.of("${abc}", "x/y", "${def}", "y/z", "${ghi}", "a/b/c"));

        assertThat(resolve(newArrayList(VariableMapping.create("${AbC}", "x\\y"),
                VariableMapping.create("${dEf}", "y\\z"), VariableMapping.create("${G_H I}", "a\\b\\c")), null))
                        .hasSize(6)
                        .containsAllEntriesOf(ImmutableMap.of("${abc}", "x\\y", "${def}", "y\\z", "${ghi}", "a\\b\\c"));
    }
    ...
    public void testResolvingMappingsWithVariablesInValues_whenAllVariablesAreKnown() {
        assertThat(resolve(newArrayList(VariableMapping.create("${A}", "x/y"), VariableMapping.create("${B}", "${A}/z"),
                VariableMapping.create("${C}", "${A}/${A}")), null)).hasSize(6)
                        .containsAllEntriesOf(ImmutableMap.of("${a}", "x/y", "${b}", "x/y/z", "${c}", "x/y/x/y"));

        assertThat(resolve(newArrayList(VariableMapping.create("${A}", "x\\y"),
                VariableMapping.create("${B}", "${A}\\z"), VariableMapping.create("${C}", "${A}\\${A}")), null))
                        .hasSize(6)
                        .containsAllEntriesOf(ImmutableMap.of("${a}", "x\\y", "${b}", "x\\y\\z", "${c}", "x\\y\\x\\y"));

        assertThat(resolve(newArrayList(VariableMapping.create("${Abc}", "x"),
                VariableMapping.create("${dEf}", "${A B C}y"), VariableMapping.create("${GHI}", "${D_ e_ F_}z")), null))
                        .hasSize(6)
