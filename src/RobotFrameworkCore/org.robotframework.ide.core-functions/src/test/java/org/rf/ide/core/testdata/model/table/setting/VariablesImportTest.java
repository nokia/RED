...
public class VariablesImportTest {

    private RobotToken decToken;
    private VariablesImport testable;

    @Before
    public void setUp() {
        this.decToken = new RobotToken();
        this.testable = new VariablesImport(decToken);
    }
    ...
    public void test_ifCellWillBeAdded_atArgumentPosition() {
        ...
        assertThat(elementTokens.get(0).getTypes()).contains(RobotTokenType.SETTING_VARIABLES_DECLARATION);
        assertThat(elementTokens.get(1).getText()).isEqualTo("value");
        assertThat(elementTokens.get(1).getTypes()).contains(RobotTokenType.SETTING_VARIABLES_ARGUMENT);
    ...
    public void test_ifCellWillBeAdded_atCommentPosition() {
        ...
        assertThat(elementTokens.get(2).getTypes()).contains(RobotTokenType.SETTING_VARIABLES_ARGUMENT);
        assertThat(elementTokens.get(3)).isSameAs(cmtToken);
        assertThat(elementTokens.get(3).getTypes()).contains(RobotTokenType.START_HASH_COMMENT);
    }
}
