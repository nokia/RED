...
public class KeywordTeardownNameMapper implements IParsingMapper {
    ...
    static boolean hasKeywordNameAlready(final List<LocalSetting<UserKeyword>> teardowns) {
        return !teardowns.isEmpty() && teardowns.get(teardowns.size() - 1)
                .getToken(RobotTokenType.KEYWORD_SETTING_TEARDOWN_KEYWORD_NAME) != null;
    }
    ...
    public RobotToken map(final RobotLine currentLine, final Stack<ParsingState> processingState,
        ...
        final UserKeyword keyword = keywords.get(keywords.size() - 1);
        final List<LocalSetting<UserKeyword>> teardowns = keyword.getTeardowns();
        final LocalSetting<UserKeyword> keywordTeardown = teardowns.get(teardowns.size() - 1);
        keywordTeardown.addToken(rt);
