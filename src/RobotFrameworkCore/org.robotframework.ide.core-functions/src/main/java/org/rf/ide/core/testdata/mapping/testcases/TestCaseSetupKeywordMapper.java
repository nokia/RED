...
public class TestCaseSetupKeywordMapper implements IParsingMapper {
    ...
    static boolean hasKeywordNameAlready(final List<LocalSetting<TestCase>> setups) {
        return !setups.isEmpty()
                && setups.get(setups.size() - 1).getToken(RobotTokenType.TEST_CASE_SETTING_SETUP_KEYWORD_NAME) != null;
    }
    ...
    public RobotToken map(final RobotLine currentLine, final Stack<ParsingState> processingState,
        ...
        final LocalSetting<TestCase> testSetup = setups.get(setups.size() - 1);
        testSetup.addToken(rt);
