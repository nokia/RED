...
public class RobotTestExecutionContext {
    ...
    public void startSuite(String longname) {
        suiteMap.put(new RobotSuiteExecutionContext(longname), ExecutionStatus.RUNNING);
    }

    public void startTest(String longname) {
        RobotTestCaseExecutionContext newTestCase = new RobotTestCaseExecutionContext(longname);
        ...
        for (RobotSuiteExecutionContext suiteExecutionContext : suiteSet) {
            if (suiteExecutionContext.getLongname().equals(suiteName)) {
                suiteExecutionContext.getTestCaseMap().put(newTestCase, ExecutionStatus.RUNNING);
                break;
            }
        }
    ...
    public void startKeyword(String name) {
        ...
        if (!currentKeywords.isEmpty()) {
            ...
            RobotTestCaseExecutionContext currentTestCase = currentTestCases.getLast();
            currentTestCase.getKeywordMap().put(newKeyword, ExecutionStatus.RUNNING);
        }
    ...
    public void endKeyword(String name) {
        if (currentKeywords.size() > 1) {
            ...
            RobotTestCaseExecutionContext currentTestCase = currentTestCases.getLast();
            currentTestCase.getKeywordMap().put(currentKeywords.getLast(), ExecutionStatus.EXECUTED);
        }
    ...
    public void endTest(String longname) {
        ...
        for (RobotSuiteExecutionContext suiteExecutionContext : suiteSet) {
            if (suiteExecutionContext.getLongname().equals(suiteName)) {
                Set<RobotTestCaseExecutionContext> testCaseSet = suiteExecutionContext.getTestCaseMap().keySet();
                for (RobotTestCaseExecutionContext testCaseExecutionContext : testCaseSet) {
                    if (testCaseExecutionContext.getLongname().equals(longname)) {
                        suiteExecutionContext.getTestCaseMap().put(testCaseExecutionContext, ExecutionStatus.EXECUTED);
                        break;
                    }
                }
            }
